...
<html lang="en">
 ...
 <center><h2>Approved Students List of CV</h2></center>
...
$sql = "SELECT * FROM student_details where Approve='1' and Branch='CV' LIMIT $start_from, $num_rec_per_page"; 
...
$num_rec_per_page=2;
$connect_my=mysqli_connect('localhost','id18835262_tpo','Admin@123456','id18835262_tpo_poornima');
$sql_my = "SELECT * FROM student_details where Approve='1' and Branch='CV'"; 
$rs_result_my =$connect_my->query ($sql_my); //run the query
$total_records = mysqli_num_rows($rs_result_my);  //count number of records
		...
		if( $currentpage > 1 && $currentpage <= $totalpage)
			{
				...
				$prev = $currentpage-1;
				echo "<li><a  href='cv.php?page=".$prev."'><</a></li>";
...
$prev = $currentpage-1;
	for ($i=$prev+1; $i<=$currentpage+2; $i++){
		echo "<li><a href='cv.php?page=".$i."'>".$i."</a></li>";
  }
  }
	...
	{
		$nxt = $currentpage+1;
		echo "<li><a  href='cv.php?page=".$nxt."' >></a></li>";
	}
