...
// mysql_select_db("id18835262_tpo_poornima"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur)
	{
		...
		{
			$sql = $connect->query("SELECT * FROM `id18835262_tpo_poornima`.`tpo_login` WHERE `Username`='$Username'");
			if($sql->num_rows == 1)
			{
				$row = $sql->fetch_assoc();
				...
				if($cur == $dbpassword)
				{
					if($query = $connect->query("UPDATE `id18835262_tpo_poornima`.`tpo_login` SET `Password` = '$Password' WHERE `tpo_login`.`Username` = '$Username'"))
					{
