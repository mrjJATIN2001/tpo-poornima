...
{
$Sname = $_POST['sname'];
$RESULT = $connect->query("SELECT count(*) FROM student_details WHERE  `Approve`='1' AND (`FirstName` LIKE '%$Sname%' or `LastName` LIKE '%$Sname%')");
$data=$RESULT->fetch_assoc();
echo "<br><h3>Number of Students with Name '$Sname'&nbsp:&nbsp";
echo $data['count(*)'];
echo "</h3>";
$sql = $connect->query("SELECT * FROM student_details WHERE `Approve`='1' AND (`FirstName` LIKE '%$Sname%' or `LastName` LIKE '%$Sname%')");
...
<p>Copyright &copy; 2022 PIET-PGC
            | Developed by <a href="#" target="_parent">Code Brigade </a></p>
          </footer>
        </div>
      </div>
    </div>
